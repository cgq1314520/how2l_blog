...
public class commentDaoImpl implements commentDao {
    ...
    public ArrayList<ArrayList<Comment>> getComment(Connection connection, int pkBid) {
        ...
        if (connection != null) {
            try {
                ...
                String sql = "SELECT b.*,c.username,c.avatar FROM (SELECT * FROM blog.comment WHERE id=?) AS b,blog.user AS c WHERE b.parent=0 AND b.`uid`=c.pk_uid";
                //开始查询
        ...
        for (ArrayList<Comment> comments : result) {
            try {
                ...
                String sql = "SELECT DISTINCT  b.*,c.username,c.avatar FROM(SELECT * FROM comment WHERE comment.`parent`=? AND comment.id=?) AS b,user AS c WHERE b.`uid`=c.pk_uid ";
                //开始执行查询语句进行查询
    ...
    public int insertComment(Connection connection, int pkBid, long uid, String comment, int parent) {
        ...
        int result = 0;
        if (connection != null) {
            String sql = "INSERT INTO comment(id,uid,parent,content) VALUE(?,?,?,?)";
            try {
